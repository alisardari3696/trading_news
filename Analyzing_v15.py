...
TP_VALUES = [x / 1000 for x in range(2, 9)]
SL_VALUES = [x / 1000 for x in range(2, 9)]
...
def main():
    ...
    all_df_sorted = all_df.sort_values("beta_weighted_ev_R", ascending=False)

    group_cols = ["pair", "sub_mode"] if mode == "candle_colour" else ["pair"]

    best_df = (
        all_df_sorted.sort_values(
            group_cols + ["beta_weighted_ev_R"],
    ...
    output_file = f"{group}_{clean_time}_v15.xlsx"
    ...
    print(f"Best rows sorted by Beta-Weighted EV:")
    print(best_df[["pair", "tp_percent", "sl_percent", "risk_to_reward_ratio",
                     "win_rate_percent", "trade_count", "ci_lower", "ci_upper",
                     "beta_weighted_ev_R"]].to_string(index=False))
