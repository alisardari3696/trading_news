...
OUTPUT_EXCEL_FILE = "News_Grid_Search_Results_v4.xlsx"
...
def load_news_data(event_currency, entry_time_str, inverted_event, mode="surprise"):
    ...
    if mode == "candle_colour":
        ...
        for col in ["Actual", "Forecast"]:
            df[col] = pd.to_numeric(
                df[col].astype(str).apply(parse_value),
                errors="coerce",
            )
...
def summarize_trades(trades, mode, sub_mode, pair, tp, sl):
    ...
    # Calculate semivariance (downside risk) - variance of negative returns
    negative_returns = tdf[tdf["pnl_pct"] < 0]["pnl_pct"]
    semivariance = negative_returns.var() * 100 if not negative_returns.empty else 0
...
def main():
    ...
    news_df = load_news_data(group, entry_time, inverted_event, mode)
    ...
    # Sort All_Results sheet by highest total pnl to lowest
